...
<?
	echo "<div class='btn_pagina'>
			<div class='btn_cadastro'>
				<a href='cad.php'><p>Cadastar Bairro</p></a>
			</div>    
		</div>";

	echo "<div class='topo_pagina'>
			<div class='titulo_pagina'>Bairros cadastrados </div>    
		</div>";


	

$sql = "SELECT * FROM $banco ORDER BY id_cidade DESC";
...
else
{
	...
	echo "<div class='titulo'>
			<div class='tituloestado'> Cidade </div>    
	...
	while ($linha2=mysql_fetch_array($resultado)) {
		$nome = $linha2["nome"];
		$id = $linha2["id"];
		$id_cidade = $linha2["id_cidade"];
		
		$sql2 = "SELECT * FROM $banco2 where id = $id_cidade ORDER BY nome DESC";
		...
		echo "<div class='item' style='background-color:$color; cursor:default' onMouseOver=\"javascript:this.style.backgroundColor='#EFEFEF'\" onMouseOut=\"javascript:this.style.backgroundColor='$color'\">
				<div class='itemestado'> $nomecidade</div>    
