...
if($_GET['acao'] == "cad"){
			...
			$query = "INSERT INTO $banco (id,nome,id_estado ) VALUES ('null','".$_POST['nome']."','".$_POST['estado']."');";
...
if($_GET['acao'] == "alt"){
			...
			$query = "UPDATE $banco SET nome='".$_POST['nome']."', id_estado='".$_POST['estado']."' WHERE id='".$_GET['id']."'";
...
<?
	echo "<div class='btn_pagina'>
			<div class='btn_listar'>
				<a href='vis.php'><p>Listar Cidades</p></a>
			</div>    
		</div>";

	echo "<div class='topo_pagina'>
			<div class='titulo_pagina'> Cadastro de Cidades </div>    
		</div>";
     ...
     echo "<div class='formulario'>
			...
			<div class='item_select_formulario'>
				<select name='estado' id='estado' class='validate[required] item_normal_formulario'>";
                	$sql = "SELECT * FROM $banco2 ORDER BY nome";
                    $res = mysql_query( $sql );
			 ...
			 <div class='item_select_ajuda'>
				<img src='../images/ajuda.png' onclick=\"$.jGrowl('Você deve selecionar a qual estado pertence esta cidade.', { header: 'Estados', position:'center', life: 100000});\" />
         	</div>
