...
if($_GET['acao'] == "cad"){
			...
			$query = "INSERT INTO $banco (id, fixo, nome ) VALUES ('null','1','".$_POST['nome']."');";
...
if($_GET['acao'] == "alt"){
			...
			if ($_GET['fixo'] == '0'){
				?>
				<script>
				   window.alert("Esta informação não pode ser alterada!");
				</script>
                <meta HTTP-EQUIV = "Refresh" CONTENT = "0; URL = vis.php">
				<?
				exit();
...
if($_GET['acao'] == "exc_"){	
			...
			if ($_GET['fixo'] == '0'){
				?>
				<script>
				   window.alert("Esta informação não pode ser exluida!");
				</script>
                <meta HTTP-EQUIV = "Refresh" CONTENT = "0; URL = vis.php">
				<?
			 exit();
...
<?
	...
	echo "<div class='topo_pagina'>
			...
			if($_GET['acao'] == "alt_" or $_GET['acao'] == "alt"){
				echo "<form id='formcadastro' name='formcadastro' method='POST' action='cad.php?acao=alt&id=".$_GET['id']."&fixo=".$_GET['fixo']."' enctype='multipart/form-data'>";
			}
